...
import { ATTENDANCE_EMPTY, ATTENDANCE_ERROR, ATTENDANCE_FILTERED } from '../fixtures/attendance';
...
test.describe('Attendance', () => {
  ...
  test('should display attendance data table', async ({ page }) => {
    await page.goto('/attendance');
    await page.waitForLoadState('networkidle');
  ...
  test('should close create modal on cancel', async ({ page }) => {
    ...
    if (await addButton.isVisible()) {
      await addButton.first().click();
      await expect(page.locator('[role="dialog"]')).toBeVisible({ timeout: 5000 });
      const cancelButton = page.locator('[role="dialog"] button:has-text("Batal")');
  ...
  test('should search and filter table results', async ({ page }) => {
    await page.goto('/attendance');
    await page.waitForLoadState('networkidle');
    const searchInput = page.locator('[data-tour="quick-search"] input');
    if (await searchInput.isVisible()) {
      await searchInput.fill('ATT001');
    }
  });
  ...
  test('should show empty state when no data', async ({ page }) => {
    await page.route(/^http:\/\/localhost:3000\/api\/attendance/, async (route) => {
      await route.fulfill({ json: ATTENDANCE_EMPTY });
    });
    await page.goto('/attendance');
    await page.waitForLoadState('networkidle');
    await expect(page.locator('[data-tour="data-table"]')).toBeVisible({ timeout: 10000 });
  ...
  test('should show error message on API failure', async ({ page }) => {
    await page.route(/^http:\/\/localhost:3000\/api\/attendance/, async (route) => {
      await route.fulfill({ status: 500, json: ATTENDANCE_ERROR });
    });
    await page.goto('/attendance');
    await page.waitForLoadState('networkidle');
  });
  ...
  test('should verify filtered data displays only matching results', async ({ page }) => {
    await page.route(/^http:\/\/localhost:3000\/api\/attendance/, async (route) => {
      await route.fulfill({ json: ATTENDANCE_FILTERED });
    });
    await page.goto('/attendance');
    await page.waitForLoadState('networkidle');
    await expect(page.locator('[data-tour="data-table"]')).toBeVisible({ timeout: 10000 });
