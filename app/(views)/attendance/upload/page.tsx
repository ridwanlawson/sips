    ...
    try {
      ...
      const createPayloadItem = (record: AttendanceUploadData) => {
        ...
        return {
          ...
          document_classification: record.document_classification ? Number(record.document_classification) : null,
      ...
      for (let i = 0; i < totalRecords; i += BATCH_SIZE) {
        ...
        setSubmitProgress(`Submitting batch ${currentBatchNum}/${totalBatches} (${Math.min(i + BATCH_SIZE, totalRecords)}/${totalRecords})...`);
  ...
  return (
    <div className="min-h-screen bg-base-100 p-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-4">
            ...
            {searchTerm && (
              <p className="text-xs text-base-content/60 mt-2">
                Menampilkan {filteredData.length} dari {data.length} records
              </p>
            )}
          </div>
        )}
        ...
        {/* Empty search result */}
        {data.length > 0 && searchTerm && filteredData.length === 0 && (
          <div className="alert mb-4 shadow-sm bg-base-200 border border-base-300">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
        ...
        {data.length > 0 && filteredData.length > 0 && (
          <div className="bg-base-100 rounded-xl shadow-sm overflow-x-auto border border-base-200">
            <table className="w-full text-sm">
              <thead className="bg-base-200 border-b border-base-300 sticky top-0">
                <tr>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-base-content">
                    No
                  </th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-base-content">
                    ID
                  ...
                  <th className="px-3 py-2 text-center text-xs font-semibold text-base-content">
                    Mandays
                  </th>
                  <th className="px-3 py-2 text-center text-xs font-semibold text-base-content">
                    Othrs
              ...
              <tbody className="divide-y divide-base-200">
                {filteredData.map((record, idx) => (
                  <tr
                    key={`${record.linenokey}-${record.id}`}
                    className="hover:bg-base-200 text-xs"
                  >
                    <td className="px-3 py-2 text-base-content">{idx + 1}</td>
                    <td className="px-3 py-2 text-base-content">{record.id}</td>
                    ...
                    <td className="px-3 py-2 text-base-content whitespace-nowrap">
                      {new Date(record.fddate).toLocaleDateString("id-ID")}
                    </td>
                    <td className="px-3 py-2 text-base-content">
                      {record.afdeling}
                    </td>
                    ...
                    <td className="px-3 py-2 text-center text-base-content">
                      {record.mandays}
                    </td>
                    <td className="px-3 py-2 text-center text-base-content">
                      {record.othrs}
                    ...
                    <td className="px-3 py-2 text-base-content max-w-xs truncate">
                      {record.remarks}
                    </td>
                    <td className="px-3 py-2 text-base-content max-w-xs truncate">
                      {record.reference || "-"}
