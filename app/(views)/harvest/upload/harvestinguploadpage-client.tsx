...
const EMPTY_PARAMS: HarvestingUploadParams = {
  nospb: "", tanggal: "", tanggal_end: "",
  kode_kendaraan: "", kode_karyawan_driver: "",
  mill: "", fcba: "", chitno: "",
};
...
export default function HarvestingUploadPage() {
  ...
  const summary = useMemo(() => {
    ...
    const totalEstateWeight = filteredDataWithKey.reduce((s, r) => s + (Number(r.bunch_estateweight) || 0), 0);
    return {
      count: filteredDataWithKey.length,
      totalBunch,
      avgBunch: filteredDataWithKey.length > 0 ? (totalBunch / filteredDataWithKey.length).toFixed(2) : 0,
      totalEstateWeight,
    };
  }, [filteredDataWithKey]);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-base-content/60 mt-1">Tampilkan dan upload data panenan TBS dari external API</p>
          </div>
          ...
          <div className="bg-base-100 p-4 rounded-xl shadow-sm mb-4 border border-base-200">
            <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div className="form-group">
                ...
                <input type="text" name="fcba" placeholder="PTE, MTE, dll" value={formParams.fcba ?? ""} onChange={handleParamChange} className="input input-bordered w-full input-sm" />
              </div>
        ...
        {loading && !showFilters && <div className="alert alert-info mb-4 shadow-sm"><span className="loading loading-spinner loading-sm" /><span>Mengambil data harvesting dari server...</span></div>}
        {!loading && data.length === 0 && !error && (
          <div className="alert mb-4 shadow-sm bg-base-200 border border-base-300">
            <div><p className="font-medium">Tidak ada data harvesting</p><p className="text-sm opacity-75">Silakan gunakan filter untuk mencari data SPB panenan.</p></div>
          </div>
        )}
        ...
        <div className="flex gap-3 mb-6 items-center flex-wrap">
          <button onClick={handleSubmitHarvesting} disabled={data.length === 0 || submitting} className="btn btn-success">
        ...
        {data.length > 0 && (
          <div className="mb-4 max-w-md relative">
            <input
              type="text" placeholder="Cari No SPB, Vehicle, Driver, Mill..."
