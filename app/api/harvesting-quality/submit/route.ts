...
export async function POST(req: NextRequest) {
  try {
    console.log("========== HARVESTING QUALITY SUBMIT START ==========");
    ...
    console.log("========== HARVESTING QUALITY SUBMIT DEBUG ==========");
    ...
    const externalUrl = `${EXTERNAL_API_BASE}/api/uploads/harvestingquality`;
    ...
    console.log("✓ Success! Response data:", JSON.stringify(responseData, null, 2).substring(0, 500));
    console.log("======================================================");

    return NextResponse.json(responseData);
  } catch (error) {
    console.error("❌ Submit harvesting quality proxy error:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Failed to submit harvesting quality data: " + (error instanceof Error ? error.message : String(error)),
      },
      { status: 500 }
    );
  }
