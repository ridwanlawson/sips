...
export async function POST(req: NextRequest) {
  try {
    console.log("========== HARVESTING SUBMIT START ==========");
    ...
    console.log("========== HARVESTING SUBMIT DEBUG ==========");
    ...
    const externalUrl = `${EXTERNAL_API_BASE}/api/uploads/harvesting`;
    ...
    console.error("❌ Submit harvesting proxy error:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Failed to submit harvesting data: " + (error instanceof Error ? error.message : String(error)),
      },
      { status: 500 }
    );
  }
