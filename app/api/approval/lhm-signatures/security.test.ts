...
describe("LHM Signatures API Security", () => {
    ...
    it("should apply data scope to search parameters for restricted roles", async () => {
        ...
        const req = new NextRequest("http://localhost/api/approval/lhm-signatures?fcba=OTHER&afdeling=OTHER&kemandoran=OTHER");
    ...
    it("should return generic error message and not leak internal error details on crash", async () => {
        const req = new NextRequest("http://localhost/api/approval/lhm-signatures?fcba=A&afdeling=B&kemandoran=C");
