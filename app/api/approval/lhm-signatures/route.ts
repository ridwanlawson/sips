...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!fcba || !afdeling || !kemandoran) {
      return NextResponse.json(
        { success: false, message: "Missing required parameters: fcba, afdeling, kemandoran", data: {} },
        { status: 400 },
      );
    }
    ...
    const fetchUserByLevel = async (level: string, includeGangcode: boolean = true): Promise<string> => {
