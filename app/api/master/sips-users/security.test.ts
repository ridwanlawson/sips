...
describe('Master Users API Security', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should return security error if validateSecurity fails (Rate Limit/CSRF)', async () => {
    ...
    const req = new NextRequest('http://localhost/api/master/sips-users');
  ...
  it('should return generic error message and not leak upstream details on failure (CWE-209)', async () => {
    ...
    vi.mocked(global.fetch).mockResolvedValue({
      ok: false,
      status: 500,
      json: async () => ({ sensitive: 'Internal Server Error: Database leak at 10.1.1.2' }),
    } as Response);
    ...
    expect(data.error).toBe('Failed to fetch master users');
    expect(data.ok).toBe(false);
    // Should NOT leak sensitive upstream data
    expect(JSON.stringify(data)).not.toContain('10.1.1.2');
  });
});
